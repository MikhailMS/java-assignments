			...
			for(int x=0; x<MIN_WINDOW_SIZE_X; ++x) {
				if (pixels[i] <= 51) {
					if ( (pixels[i+1] | pixels[i-1] | pixels[i+MIN_WINDOW_SIZE_Y] 
						| pixels[i-MIN_WINDOW_SIZE_Y] )  <= 51) { // Check if the pixel to the right, left, bottom and above the same color with current pixel
					...
					if ((pixels[i+1] | pixels[i-1] | pixels[i+MIN_WINDOW_SIZE_Y]
						| pixels[i-MIN_WINDOW_SIZE_Y] ) <= 53) { // Check if the pixel to the right, left, bottom and above the same color with current pixel
						drawPixelSketch.setColor(255, 255, 255); 
						drawPixelSketch.plot(x,y);
					}
					else {
					...
					if ((pixels[i+1] | pixels[i-1] | pixels[i+MIN_WINDOW_SIZE_Y]
						| pixels[i-MIN_WINDOW_SIZE_Y] ) <= 55) { // Check if the pixel to the right, left, bottom and above the same color with current pixel
						drawPixelSketch.setColor(255, 255, 255); 
						drawPixelSketch.plot(x,y);
					}
					else {
